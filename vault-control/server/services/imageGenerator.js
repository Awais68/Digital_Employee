...
function loadImageDataUri(filePath) {
  ...
  try {
    ...
    console.log(
      `[ImageGen] Logo loaded: ${path.basename(filePath)} (${(buf.length / 1024).toFixed(0)}KB)`,
    );
  } catch (e) {
...
const EMOJI_RE =
  /[\u{1F000}-\u{1FAFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{2B00}-\u{2BFF}\u{1F1E6}-\u{1F1FF}\u{FE00}-\u{FE0F}\u{200D}\u{2049}\u{203C}]/gu;
...
function logEmojiFontStatus() {
  ...
  try {
    const out = spawnSync("fc-list", [":charset=1f600"], {
      encoding: "utf-8",
      timeout: 3000,
    });
    const has = !!(out.stdout && out.stdout.trim().length > 0);
    console.log(
      `[ImageGen] Color-emoji font on host: ${has ? "yes" : "NO"} (emoji stripped from render regardless)`,
    );
  } catch {
    /* fc-list unavailable — irrelevant since emoji are stripped */
  }
}
...
const FORBIDDEN_FRAGMENTS = [
  ...
  "Requirements:",
  "Core message:",
  "Key data/stats to visually emphasize",
...
function validateStructuredContent(content) {
  ...
  if (content.headline.length > MAX_HEADLINE_CHARS) {
    console.warn(
      `[ImageGen] Headline truncated from ${content.headline.length} to ${MAX_HEADLINE_CHARS} chars`,
    );
  ...
  for (const frag of FORBIDDEN_FRAGMENTS) {
    if (allText.includes(frag.toLowerCase())) {
      return {
        valid: false,
        error: `Forbidden fragment detected: "${frag}"`,
        content,
      };
    }
  }
  if (TEMPLATE_SYNTAX_RE.test(allText)) {
    return {
      valid: false,
      error: "Unresolved template syntax detected",
      content,
    };
  }
...
function extractContentFromPost(postContent, topic) {
  ...
  const headline = topic.replace(/^[🚀💡🔥⚡🎯📊💪🌟✅📈]\s*/, "").trim();
  ...
  const stats = statLines.slice(0, 4).map((l) => {
    ...
    const label =
      l
        ...
        .replace(/[^\w\s-]/g, "")
        .trim()
        .substring(0, MAX_STAT_LABEL_CHARS) || "Key metric";
  ...
  if (stats.length === 0) {
    const numbers =
      postContent.match(/\d+%|\$[\d,.]+[kKmMbBtT]?|\b\d+[xX]\b(?!\d+)/g) || [];
    const cleanNumbers = numbers.filter((n) => !/^\d+x\d+$/i.test(n));
    if (cleanNumbers.length > 0) {
      stats.push({
        icon: "📊",
        value: cleanNumbers[0].trim(),
        label: "Impact metric",
      });
    }
  }

  // const bulletMatchLines = lines.filter(l => l.match(/^[→✅📊💡🔥⚡\d️⃣]|^- /))
  const bulletMatchLines = lines.filter((l) =>
    l.match(/^[→✅📊💡🔥⚡]|^-\s|^\d+[.)]\s|^•/),
  );
  const usedBulletLines = bulletMatchLines.slice(0, 4);

  const bullets = usedBulletLines
    .map((l) =>
      l
        .replace(/^[→✅📊💡🔥⚡\d️⃣\-]+/, "")
        .replace(/^\s+/, "")
  ...
  function truncateWordSafe(text, maxLen) {
    if (text.length <= maxLen) return text;
    const clipped = text.substring(0, maxLen);
    const lastSpace = clipped.lastIndexOf(" ");
  ...
  const ctaLine = lines.find((l) => {
    if (usedBulletLines.includes(l)) return false;
    if (/^#/.test(l.trim())) return false; // skip hashtag lines
    if (/^https?:\/\//.test(l.trim())) return false; // skip link-only lines
    return (
      l.includes("?") ||
      l.includes("👇") ||
      l.includes("↓") ||
  ...
  const cta = ctaLine
    ? truncateWordSafe(
        ctaLine.replace(/^[🚀💡🔥⚡🎯📊💪🌟✅📈]\s*/, "").trim(),
        MAX_CTA_CHARS,
      )
    : GENERIC_CTAS[
        Math.abs(topic.split("").reduce((a, c) => a + c.charCodeAt(0), 0)) %
          GENERIC_CTAS.length
      ];
...
- Bullets: MUST be specific, concrete facts/actions taken directly from the post text (not generic advice). Do NOT invent generic statements like "Start small" unless those exact words/ideas appear in the post.
- CTA: MUST be a real question ending with "?" that is specific to the post's topic. NEVER output hashtags, links, or generic filler. If the post itself doesn't end in a clear question, invent one short specific question about the post's core topic.
...
${postContent.substring(0, 1500)}`;
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (validation.valid) {
        console.log(
          `[ImageGen] AI content extraction succeeded (attempt ${attempt + 1})`,
        );
        return validation.content;
      }
      lastError = validation.error;
      console.warn(
        `[ImageGen] AI extraction attempt ${attempt + 1} validation failed:`,
        lastError,
        "- raw:",
        raw.substring(0, 120),
      );
      ...
      lastError = e.message;
      console.warn(
        `[ImageGen] AI extraction attempt ${attempt + 1} parse failed:`,
        e.message,
      );
    }
  }
  throw new Error(
    `AI content extraction failed after 2 attempts: ${lastError}`,
  );
}
...
async function extractImageContent(postContent, topic) {
  ...
  try {
    ...
    console.warn(
      "[ImageGen] AI extraction failed too, using rule-based result:",
      e.message,
    );
    return ruleContent;
  }
...
async function generateViaWikipedia(topic, width = 1080, height = 1350) {
  ...
  if (!summaryResp.ok)
    throw new Error(`Wikipedia summary: ${summaryResp.status}`);
  ...
  if (!imgResp.ok)
    throw new Error(`Wikipedia image download: ${imgResp.status}`);
...
async function generateCanvaStyleImage(
  topic,
  postContent = "",
  width = 1080,
  height = 1350,
  ...
  if (!validation.valid) {
    console.warn(
      `[ImageGen] Content validation failed: ${validation.error} — falling back to Pollinations`,
    );
  ...
  if (!safety.safe) {
    console.warn(
      `[ImageGen] Content unsafe for rendering: ${safety.reason} — falling back to Pollinations`,
    );
  ...
  const stats = (content.stats || [])
    .map((s) => ({
      value: stripEmoji(s.value || ""),
      label: stripEmoji(s.label || ""),
    }))
    .filter((s) => s.value);
  const logoUri = getLogoDataUri();
  const brandLogoUri = getBrandLogoDataUri();

  // Compute ASNEXA logo display dimensions (fixed 48px height, width from aspect ratio)
  let brandLogoDisplayW = 170;
  const BRAND_LOGO_DISPLAY_H = 90;
  if (brandLogoUri) {
    try {
      const brandBuf = fs.readFileSync(BRAND_LOGO_PATH);
      const { default: sharp } = await import("sharp");
      const meta = await sharp(brandBuf).metadata();
      if (meta.width && meta.height) {
        brandLogoDisplayW = Math.round(
          BRAND_LOGO_DISPLAY_H * (meta.width / meta.height),
        );
      }
    } catch (e) {
      console.warn(
        "[ImageGen] Could not read brand logo dimensions, using default width:",
        e.message,
      );
    }
  }
  ...
  const hFontSize = Math.min(
    76,
    Math.max(48, Math.floor(720 / Math.sqrt(headline.length + 1)) + 10),
  );
  ...
  const hLines = [];
  {
    ...
    for (const w of headline.split(/\s+/)) {
      ...
      if (candidate.length > hMaxChars && line) {
        hLines.push(line);
        line = w;
      } else line = candidate;
    }
  ...
  const BODY_TEXT_MAX_AVAIL =
    BODY_MAX_BOX_WIDTH - BODY_PAD_X - BODY_BULLET_PAD_LEFT;
  const BODY_MAX_CHARS = Math.max(
    10,
    Math.floor(BODY_TEXT_MAX_AVAIL / BODY_CHAR_W),
  );
  ...
  const bodyWidestChars = bodyBulletLines.reduce(
    (m, l) => Math.max(m, ...l.map((s) => s.length)),
    0,
  );
  const BODY_BOX_WIDTH = bodyBullets.length
    ? Math.min(
        ...
        Math.max(
          360,
          Math.ceil(bodyWidestChars * BODY_CHAR_W) +
            BODY_PAD_X +
            BODY_BULLET_PAD_LEFT,
        ),
      )
    : 0;
  ...
  const bodyBulletH = bodyBullets.length
    ? BODY_LABEL_H +
      bodyTotalLines * BODY_BULLET_LINE_H +
      bodyBulletGaps * 12 +
      BODY_PAD_Y
    : 0;
  ...
  // ── CTA / question pill (dynamic padding — 10px base, adjusts by content) ─
  const CTA_FONT = 18;
  ...
  const CTA_MAX_CHARS = Math.max(
    14,
    Math.floor((CTA_MAX_PILL_W - 2 * CTA_PILL_PAD_X) / CTA_CHAR_W),
  );
  const ctaLines = cta ? wrapText(cta, CTA_MAX_CHARS) : [];
  const ctaLineH = 28;
  const CTA_PILL_PAD_Y =
    10 + (ctaLines.length > 1 ? 10 : 0) + (cta.length > 60 ? 10 : 0);
  const ctaPillH = ctaLines.length
    ? ctaLines.length * ctaLineH + CTA_PILL_PAD_Y
    : 0;
  ...
  // ── Big Digital FTE logo (above headline) ───────────────────────────
  const BIGLOGO_W = Math.round(Math.min(H_USABLE, 220));
  const BIGLOGO_H = Math.round(BIGLOGO_W * 0.5);
  // ── Assemble block order ──────────────────────────────────────────────
  const blocks = [];
  if (logoUri) blocks.push({ kind: "biglogo", h: BIGLOGO_H, gapAfter: 0 });
  ...
  const totalH =
    blocks.reduce((a, b) => a + b.h, 0) +
    blocks
      .slice(0, -1)
      .reduce((a, b) => a + (b.gapAfter != null ? b.gapAfter : BLOCK_GAP), 0);
  let cursorY = HEADER_BOTTOM + Math.max(0, (AVAILABLE - totalH) / 2);

  // ── Render blocks ─────────────────────────────────────────────────────
  const parts = [];
  for (const block of blocks) {
    if (block.kind === "biglogo") {
      const logoPad = 14;
      parts.push(`<rect x="${cx - BIGLOGO_W / 2 - logoPad}" y="${cursorY - logoPad}" width="${BIGLOGO_W + logoPad * 2}" height="${BIGLOGO_H + logoPad * 2}" rx="14" fill="rgba(255,255,255,0.03)" stroke="rgba(0,201,167,0.1)" stroke-width="1"/>
    <image href="${logoUri}" xlink:href="${logoUri}" x="${cx - BIGLOGO_W / 2}" y="${cursorY}" width="${BIGLOGO_W}" height="${BIGLOGO_H}" preserveAspectRatio="xMidYMid meet"/>`);
    ...
    <text x="${BODY_BOX_X + BODY_PAD_X}" y="${cursorY + 34}" fill="${ELECTRIC_TEAL}" font-family="'Poppins',sans-serif" font-size="16" font-weight="700" letter-spacing="1.4">KEY POINTS</text>`);
      ...
      bodyBulletLines.forEach((lines, bi) => {
        lines.forEach((ln, li) => {
          if (li === 0) {
            parts.push(
              `<circle cx="${BODY_BOX_X + BODY_PAD_X + 8}" cy="${runY - Math.round(BODY_BULLET_FONT * 0.32)}" r="8" fill="${LINKEDIN_BLUE}"/>`,
            );
          }
          const indent =
            BODY_BOX_X + BODY_PAD_X + BODY_BULLET_PAD_LEFT - (li === 0 ? 0 : 0);
          parts.push(
            `<text x="${indent}" y="${runY}" fill="${LIGHT}" font-family="'Inter',sans-serif" font-size="${BODY_BULLET_FONT}" font-weight="500">${escapeXml(ln)}</text>`,
          );
      ...
      const pillW = Math.min(
        CTA_MAX_PILL_W,
        Math.max(320, Math.ceil(widestCta * CTA_CHAR_W) + 2 * CTA_PILL_PAD_X),
      );
      const pillX = cx - pillW / 2;
      // Solid dark-navy pill, teal border + soft drop shadow for strong contrast
      parts.push(
        `<rect x="${pillX}" y="${pillY}" width="${pillW}" height="${ctaPillH}" rx="${ctaPillH / 2}" fill="#08233D" stroke="${ELECTRIC_TEAL}" stroke-width="2" filter="url(#ctaGlow)"/>`,
      );
      const firstBaseline = pillY + CTA_PILL_PAD_Y / 2 + CTA_FONT;
      ctaLines.forEach((ln, i) => {
        const lineY = firstBaseline + i * ctaLineH;
        parts.push(
          `<text x="${cx}" y="${lineY}" fill="${WHITE}" font-family="'Poppins',sans-serif" font-size="${CTA_FONT}" font-weight="700" text-anchor="middle">${escapeXml(ln)}</text>`,
        );
      });
    } else if (block.kind === "asnexa") {
      if (brandLogoUri) {
        parts.push(
          `<image href="${brandLogoUri}" xlink:href="${brandLogoUri}" x="${cx - Math.round(brandLogoDisplayW / 2)}" y="${cursorY}" width="${brandLogoDisplayW}" height="${BRAND_LOGO_DISPLAY_H}" preserveAspectRatio="xMidYMid meet" opacity="0.5"/>`,
        );
      } else {
        parts.push(
          `<text x="${cx}" y="${cursorY + 16}" fill="${DIM}" font-family="'Poppins',sans-serif" font-size="12" font-weight="500" text-anchor="middle" letter-spacing="3" opacity="0.5">ASNEXA</text>`,
        );
      }
    }
  ...
  <!-- Header row — all elements aligned on the same horizontal band (moved 10px down) ── -->
  <rect x="50" y="42" width="46" height="46" rx="12" fill="${ELECTRIC_TEAL}"/>
  <text x="73" y="72" fill="${DARK_NAVY}" font-family="'Poppins',sans-serif" font-size="26" font-weight="700" text-anchor="middle">D</text>
  <text x="110" y="72" fill="${WHITE}" font-family="'Poppins',sans-serif" font-size="22" font-weight="700" letter-spacing="-0.3">Digital FTE</text>
  <rect x="${cx - 65}" y="40" width="130" height="38" rx="19" fill="rgba(0,201,167,0.12)" stroke="${ELECTRIC_TEAL}" stroke-width="1.5"/>
  <rect x="${cx - 70}" y="40" width="140" height="38" rx="19" fill="rgba(0,201,167,0.12)" stroke="${ELECTRIC_TEAL}" stroke-width="1.5"/>
  <circle cx="${cx - 42}" cy="59" r="4" fill="#00E5A0"/>
  <text x="${cx + 8}" y="64" fill="${WHITE}" font-family="'Poppins',sans-serif" font-size="12" font-weight="700" letter-spacing="1.5" text-anchor="middle">AI TRENDS</text>

  <!-- ASNEXA brand logo (far right corner) ─────────────────────────── -->
  ${
    brandLogoUri
      ? `<image href="${brandLogoUri}" xlink:href="${brandLogoUri}" x="${width - brandLogoDisplayW - 40}" y="31" width="${brandLogoDisplayW}" height="${BRAND_LOGO_DISPLAY_H}" preserveAspectRatio="xMidYMid meet"/>`
      : `<text x="${width - 50}" y="62" fill="${WHITE}" font-family="'Poppins',sans-serif" font-size="20" font-weight="700" letter-spacing="2" text-anchor="end">ASNEXA</text>`
  }
...
async function generatePremiumDesignImage(topic, width = 1080, height = 1350) {
  ...
  // Word wrap functions
  const wrapLine = (wordsArr, maxLen) =>
    wordsArr.reduce((acc, word) => {
  ...
  const sFont = 26;
  const totalH =
    headlineLines.length * hFont * 1.25 +
    40 +
    subtextLines.length * sFont * 1.5;
...
export async function generatePostImage(
  ...
  if (postContent) {
    try {
      ...
      console.warn(
        `[ImageGen] Branded template skipped: ${e.message} — falling through to photo modes`,
      );
    }
  }
    ...
    if (key) {
      const res = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "user",
              content: `${topic}, professional social media image, premium quality, ${style}`,
            },
          ],
      ...
      if (msg?.images?.[0]) {
        imageUrl =
          ...
          (typeof msg.images[0].image_url === "string"
